/*
 * Título: Assistente de Voz com Resposta Visual (Contagem Corrigida)
 * Autor: Artur (UEMG) & Gemini
 * Descrição: Versão Final do projeto. Este código corrige o número de LEDs
 * a serem controlados para corresponder ao hardware físico (24 LEDs).
 * A variável LED_COUNT foi ajustada de 16 para 24.
 *
 * Biblioteca Adicional Necessária:
 * - "Adafruit NeoPixel" da Adafruit
 */
...
#define LED_COUNT  24 // Alterado de 16 para 24
...
void setupI2SMicrophone() {
    i2s_config_t i2s_config = { .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX), .sample_rate = SAMPLE_RATE, .bits_per_sample = BITS_PER_SAMPLE_MIC, .channel_format = I2S_CHANNEL_FMT_ONLY_RIGHT, .communication_format = I2S_COMM_FORMAT_STAND_I2S, .intr_alloc_flags = ESP_INTR_FLAG_LEVEL1, .dma_buf_count = 8, .dma_buf_len = 1024, };
    i2s_pin_config_t pin_config = { .bck_io_num = I2S_BCLK_PIN, .ws_io_num = I2S_LRC_PIN, .data_out_num = I2S_PIN_NO_CHANGE, .data_in_num = I2S_DATA_IN_PIN };
...
String urlEncode(String str) {
    ...
    for (int i = 0; i < str.length(); i++) {
        c = str.charAt(i);
        if (c == ' ') { encodedString += "%20"; } else if (isalnum(c) || c == '-' || c == '_' || c == '.' || c == '~') { encodedString += c; } else { encodedString += '%'; code1 = (c & 0X0F) + '0'; if ((c & 0X0F) > 9) code1 = (c & 0X0F) - 10 + 'A'; c = (c >> 4) & 0X0F; code0 = c + '0'; if (c > 9) code0 = c - 10 + 'A'; encodedString += code0; encodedString += code1; }
    }
...
void setup() {
    Serial.begin(115220);
    Serial.println("\n\nAssistente de Voz TCC - com Feedback Visual Corrigido");
